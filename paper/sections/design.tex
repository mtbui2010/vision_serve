...
The same endpoint handles all seven task types: detection,
segmentation, open-vocabulary detection, classification, depth estimation,
embedding, and planar grasp synthesis. No client-side multiplexing is required.
...
responses.
All heavy resources, such as GPU memory and session handles, are allocated during startup
and reused across requests, so no resource allocation occurs in the request hot
path.
...
\begin{figure*}[t]
  ...
  \begin{tikzpicture}[
      ...
      gatebox/.style={draw=red!70!black, line width=1pt,
                  rounded corners=3pt, minimum width=2.2cm,
                  minimum height=0.65cm, align=center, fill=red!12},
      callout/.style={draw=red!70!black, dashed, rounded corners=3pt,
  ...
  \node[gatebox, above=0.5cm of engine] (registry){Registry\\\textbf{License Gate}};
  ...
  %% Contribution callout for the load-time license gate
  \node[callout, above left=0.45cm and -0.2cm of registry] (gatenote)
        {load-time license policy:\\allowlist + sha256 + ledger};
  \draw[dashed, red!70!black] (gatenote) -- (registry.north west);
  ...
  \caption{High-level architecture of VisionServe.
           ...
           The Registry (highlighted) enforces the load-time license gate
           --- the paper's primary contribution --- validating each model's
           license against the permissive allowlist and, in verified mode,
           binding license to sha256, source, and provenance ledger.}
...
\texttt{task} (one of the seven task types),
...
while Grounded-SAM declares \texttt{det}, \texttt{encoder}, and \texttt{decoder}.
...
This split keeps model packages testable in isolation, since unit tests exercise
pre/postprocess without an ONNX session, and it keeps the hot path in the
...
The pipeline model never creates or frees sessions; it only
names them.
This design supports arbitrarily deep chains, such as GroundingDINO boxes flowing
...
All seven task types return a single \texttt{Result} struct:
\begin{center}
\small
...
original image coordinates. The postprocess step uses the
