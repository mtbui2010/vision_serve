...
$\to$ DirectML $\to$ OpenVINO $\to$ CPU) lets the same binary exploit the best
...
deployments impose even before any model weights are loaded. That footprint reduction
matters on boards with 4--8\,GB of unified memory.

As one example, rf-detr-nano runs on a Jetson AGX Orin at 112.9\,rps under the TensorRT
EP. This is $\sim$20$\times$ the 5.6\,rps CPU path and $\sim$12$\times$ more energy-efficient
(338 vs.\ 4136\,mJ/inference), measured on-device with the same harness
(Section~\ref{sec:eval:edge}, Table~\ref{tab:edge}). The same model falls back gracefully to the
CUDA EP on a workstation GPU and to the CPU EP on a plain ARM board, driven by the manifest's
\texttt{tensorrt}\,$\to$\,\texttt{cuda}\,$\to$\,\texttt{cpu} chain with no configuration change
...
caller, since the server returns a single unified \texttt{Result} containing both the
...
\begin{enumerate}
    ...
    \item Call
          \texttt{depths = get\_depth\_at\_detection(\allowbreak depth\_result, det\_result)},
...
\subsection{Robotic Grasp Synthesis}
\label{subsec:grasp}

The \texttt{grasp} task turns a segmented object into actionable planar parallel-jaw grasps for
pick-and-place robots, in the same single HTTP request and unified \texttt{Result} schema.
Internally the \texttt{PipelineModel} routes the image through MobileSAM (class-agnostic
whole-image automask, or a detector $\to$ box-prompted mask for the class-aware
\texttt{grasp-rfdetr} / \texttt{grasp-gd} variants), then a pure-Go analytic stage
(\texttt{internal/grasp}) converts each binary mask into grasp rectangles: it builds a boundary
normal map with a Sobel-like convolution, decimates the boundary on a polar grid about the
centroid, enumerates antipodal finger pairs, and scores each candidate with a friction-cone
force-closure test. Each grasp is returned as $(x,y,\theta,\text{width},\text{quality})$ in
original-image coordinates, alongside the source mask:

{\footnotesize\begin{verbatim}
curl -X POST http://localhost:11435/api/predict \
  -F "model=grasp" \
  -F "image=@bin.jpg" \
  -F "gripper_min=10" -F "gripper_max=150"
\end{verbatim}}

\noindent
Because the synthesizer is analytic, with no neural network, no ONNX session, and no weights, it adds a
robotics capability without a new model download or any license obligation, and the gripper
opening bounds (\texttt{gripper\_min}/\texttt{gripper\_max}, in pixels) are tunable per request.
This is a deliberate license-driven design choice: it sidesteps the non-commercially-licensed
learned grasp models (Dex-Net) that VisionServe's allowlist would reject
(Section~\ref{sec:catalog:inventory}).
...
\texttt{scrfd} model identifier, with no task-specific client code needed. Both examples
show that the architecture accommodates heterogeneous model families while
