...
Counter-intuitively, VisionServe's \emph{JPEG-in} peak goodput (695~rps, Section~\ref{sec:eval:latency})
exceeds its \emph{tensor-in} peak (563~rps).  Pure-Go JPEG decode is CPU-heavy but
...
OS threads and avoids a process-level interpreter lock, but we are careful not to
overstate this: a single ONNX Runtime session in VisionServe is itself mutex-serialised
(\texttt{engine.Session.Run} holds a per-session lock), so concurrent requests routed to
the same session run one at a time, just as a GIL would serialise them.  Real
intra-process concurrency comes from the per-role \emph{session pool}
(\texttt{engine.SessionPool}): when a role is configured with a pool size $>1$, the
\texttt{lifecycle.Manager} holds several identical sessions and dispatches each request
to the first free one, so up to $N$ inferences proceed in parallel with no shared lock.
The pool size is configurable per role and defaults to~1; we therefore present this as a
configurable concurrency mechanism rather than unbounded parallel inference, and we leave
a concurrency sweep over pool size to future work.
